...
function About()
{
    ...
    useEffect(()=>{
        if(message!="")
        {
            if(message == "Record Removed Successfully!")
            {
                ...
                helper.open("GET","http://localhost:9999/product");
                helper.send();
            }
            setTimeout(() => 
            {
    ...
    const removeRecord =(Id)=>{
        ...
        helper.open("DELETE","http://localhost:9999/product/" + Id);
        helper.send();
    }

    return (
       <div>
        <button className="margin btn btn-primary" onClick={navigateToAdd}>Add Product</button>
